...
function params = parameters_TNBC_mets_w60(TumorArray,cancer_clones,n_T_specs)
...
for j=1:ncancer_clones
    params.(['ncells_' cancer_clones{j}]).Value = 4.7e6*clone_fractions(j); %4.7e6*clone_fractions(j)
...
% Death Rate
for nc=1:ncancer_clones
    params.(['k_' cancer_clones{nc} '_death']).Value = 0.0001; 
...
for nt=1:n_T_specs 
    params.(['n_clones_tum' num2str(nt)]).Value = 63*T_specs_fractions(nt); 
...
for i=2:3
    params.(['k_CD8_mig' TumorArray{i}]).Value = 3.5e-12; 
...
for i=2:3
    params.(['APC0_T' TumorArray{i}]).Value = 1.0e5; 
    params.(['APC0_T' TumorArray{i}]).Units = 'cell/milliliter';
    params.(['APC0_T' TumorArray{i}]).Notes = 'estimated'; 
end
for i=4:4
    params.(['APC0_T' TumorArray{i}]).Value = 1.0e5; 
    params.(['APC0_T' TumorArray{i}]).Units = 'cell/milliliter';
