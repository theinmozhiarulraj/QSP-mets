...
function params = parameters_TNBC_mets_w60_vct(TumorArray,cancer_clones,n_T_specs)
...
% self Ag concentrations
for i=1:ncancer_clones

    params.(['agconc_' cancer_clones{i} '_0']).Value = 5.4e-13; %4.7e6*clone_fractions(j)
    params.(['agconc_' cancer_clones{i} '_0']).Units = 'mole/cell';
    params.(['agconc_' cancer_clones{i} '_0']).Notes = 'based on previously used total concentration';

end


% neo Ag concentrations
for i=1:ncancer_clones

    for j=1:n_T_specs

        params.(['agconc_' cancer_clones{i} '_' num2str(j)]).Value = 5.4e-13/n_T_specs; %4.7e6*clone_fractions(j)
        params.(['agconc_' cancer_clones{i} '_' num2str(j)]).Units = 'mole/cell';
        params.(['agconc_' cancer_clones{i} '_' num2str(j)]).Notes = 'based on previously used total concentration';
...
for j=1:ncancer_clones
    params.(['ncells_' cancer_clones{j}]).Value = 0; %4.7e6*clone_fractions(j)
    params.(['ncells_' cancer_clones{j}]).Units = 'cell';
    params.(['ncells_' cancer_clones{j}]).Notes = 'assumption';
end
...
for i=2:ntumors
    ...
    clone_fractions=sample_fractions(ncancer_clones); 
    for j=1:ncancer_clones
        params.(['ncells_' cancer_clones{j} TumorArray{i}]).Value = 4.7e6*clone_fractions(j); %4.7e6*clone_fractions(j)
        params.(['ncells_' cancer_clones{j} TumorArray{i}]).Units = 'cell';
        params.(['ncells_' cancer_clones{j} TumorArray{i}]).Notes = 'assumption';
    end
end

% thein
for i=2:ntumors
    params.(['delay' TumorArray{i}]).Value = 2100;
    params.(['delay' TumorArray{i}]).Units = 'day';
    params.(['delay' TumorArray{i}]).Notes = 'Time delay of met seeding';
end

% Initial Tumour Diameter of atleast 1 met
params.initial_met_diameter.Value = 1.6; % 0.2 cm =2mm
params.initial_met_diameter.Units = 'centimeter';
params.initial_met_diameter.Notes = '(varied)';

% %thein
% params.delay_Ln1.Value = 100;
% params.delay_Ln1.Units = 'day';
% params.delay_Ln1.Notes = 'Time delay of met seeding';
% 
% params.delay_Ln2.Value = 20;
% params.delay_Ln2.Units = 'day';
% params.delay_Ln2.Notes = 'Time delay of met seeding';
% 
% params.delay_Ln3.Value = 50;
% params.delay_Ln3.Units = 'day';
% params.delay_Ln3.Notes = 'Time delay of met seeding';
% 
% params.delay_Ln4.Value = 120;
% params.delay_Ln4.Units = 'day';
% params.delay_Ln4.Notes = 'Time delay of met seeding';

params.surgery_time.Value = 200;
params.surgery_time.Units = 'day';
params.surgery_time.Notes = 'Time delay of surgery';
...
% Number of Lymph Nodes - thein
params.nLNs_Ln1.Value = 17;
params.nLNs_Ln1.Units = 'dimensionless';
params.nLNs_Ln1.Notes = '(estimated)';
params.nLNs_Ln2.Value = 17;
params.nLNs_Ln2.Units = 'dimensionless';
params.nLNs_Ln2.Notes = '(estimated)';
...
% Lymph Nodes Compartment Volume - thein
params.V_LN_Ln1.Value = [];
params.V_LN_Ln1.Units = 'milliliter';
params.V_LN_Ln1.Notes = ['Volume of lumped tumor-draining lymph nodes calculated based on number of LNs ' params.nLNs.Notes ' and average LN diameter ' params.D_LN.Notes];
params.V_LN_Ln1.Factors = ["nLNs_Ln1", "D_LN"];
params.V_LN_Ln1.Equation = 'p(1)*4/3*pi*(p(2)/2)^3';
params.V_LN_Ln2.Value = [];
params.V_LN_Ln2.Units = 'milliliter';
params.V_LN_Ln2.Notes = ['Volume of lumped tumor-draining lymph nodes calculated based on number of LNs ' params.nLNs.Notes ' and average LN diameter ' params.D_LN.Notes];
params.V_LN_Ln2.Factors = ["nLNs_Ln2", "D_LN"];
params.V_LN_Ln2.Equation = 'p(1)*4/3*pi*(p(2)/2)^3';
...
for nc=1:ncancer_clones
    params.(['k_' cancer_clones{nc} '_growth']).Value = 0.0065;
    params.(['k_' cancer_clones{nc} '_growth']).Units = '1/day';
    params.(['k_' cancer_clones{nc} '_growth']).Notes = '(Ryu 2014, PMID: 24895040; Desai 2006, PMID: 16489089)';
end

for i=2:ntumors
    for nc=1:ncancer_clones
        params.(['k_' cancer_clones{nc} '_growth' TumorArray{i}]).Value = 0.0065;
        params.(['k_' cancer_clones{nc} '_growth' TumorArray{i}]).Units = '1/day';
        params.(['k_' cancer_clones{nc} '_growth' TumorArray{i}]).Notes = '(Ryu 2014, PMID: 24895040; Desai 2006, PMID: 16489089)';
    end
end
% Growth Rate - thein
% params.k_C_growth_Ln1.Value = 0.0075;
% params.k_C_growth_Ln1.Units = '1/day';
% params.k_C_growth_Ln1.Notes = '(Ryu 2014, PMID: 24895040; Desai 2006, PMID: 16489089)';
...
% Death Rate
for nc=1:ncancer_clones
    params.(['k_' cancer_clones{nc} '_death']).Value = 0.0001; %thein:0.0001
    params.(['k_' cancer_clones{nc} '_death']).Units = '1/day';
    params.(['k_' cancer_clones{nc} '_death']).Notes = '(Palsson 2013, PMID: 24074340)';
end

for i=2:ntumors
    for nc=1:ncancer_clones
        params.(['k_' cancer_clones{nc} '_death' TumorArray{i}]).Value = 0.0001;
        params.(['k_' cancer_clones{nc} '_death' TumorArray{i}]).Units = '1/day';
        params.(['k_' cancer_clones{nc} '_death' TumorArray{i}]).Notes = '(Palsson 2013, PMID: 24074340)';
    end
end
...
params.initial_tumour_diameter.Value = 0; % 1 - 5 cm % thein:2.5
...
for nt=1:n_T_specs 
    params.(['n_clones_tum' num2str(nt)]).Value = 63*T_specs_fractions(nt); %thein 63
    params.(['n_clones_tum' num2str(nt)]).Units = 'dimensionless';
    params.(['n_clones_tum' num2str(nt)]).Notes = '(Morisaki 2021 PMID: 34193879; Narang 2019, PMID: 30832597)';
end
...
%thein
for i=2:3
    params.(['k_CD8_mig' TumorArray{i}]).Value = 3.5e-12; % thein fitted: 3.5e-12 %5.8e-12
    params.(['k_CD8_mig' TumorArray{i}]).Units = '1/minute/cell';
    params.(['k_CD8_mig' TumorArray{i}]).Notes = 'estimated';
end

for i=4:4
    params.(['k_CD8_mig' TumorArray{i}]).Value = 5.0e-12; % 5.8e-12
    params.(['k_CD8_mig' TumorArray{i}]).Units = '1/minute/cell';
    params.(['k_CD8_mig' TumorArray{i}]).Notes = 'estimated';
end
...
for i=1:ntumors
    params.(['q_CD8_T_in' TumorArray{i}]).Value = [];
    params.(['q_CD8_T_in' TumorArray{i}]).Units = '1/(centimeter^3*minute)';
    params.(['q_CD8_T_in' TumorArray{i}]).Notes = ['calculated based on T cell transmigration rate ' params.(['k_CD8_mig' TumorArray{i}]).Notes ' and T cell adhesion density ' params.rho_adh.Notes];
    params.(['q_CD8_T_in' TumorArray{i}]).Factors = [['k_CD8_mig' TumorArray{i}], "rho_adh", "gamma_T"];
    params.(['q_CD8_T_in' TumorArray{i}]).Equation = 'p(1)*p(2)*p(3)';
end
...
params.k_C_Tcell.Value = 0.95; %thein: 0.9 to 0.75
...
% APC Density in lung tumor
for i=2:3
    params.(['APC0_T' TumorArray{i}]).Value = 1.0e5; %thein fitted 2.0e5 %4.0e5
    params.(['APC0_T' TumorArray{i}]).Units = 'cell/milliliter';
    params.(['APC0_T' TumorArray{i}]).Notes = 'estimated'; %thein
end
for i=4:4
    params.(['APC0_T' TumorArray{i}]).Value = 1.0e5; %4.0e5
    params.(['APC0_T' TumorArray{i}]).Units = 'cell/milliliter';
    params.(['APC0_T' TumorArray{i}]).Notes = 'estimated'; %thein
end
% APC Density in LN
...
% thein: newly added ones in new file - uncomment this
% CD80-CD80 kd
...
%thein: newly added parameter in new file - uncomment
% CD80-CD80 koff
...
for i=2:3
    params.(['k_MDSC_mig' TumorArray{i}]).Value = 1.1e4;
    params.(['k_MDSC_mig' TumorArray{i}]).Units = 'cell/(milliliter*day)';
    params.(['k_MDSC_mig' TumorArray{i}]).Notes = 'estimated';
end

for i=4:4
    params.(['k_MDSC_mig' TumorArray{i}]).Value = 6.5e4;
    params.(['k_MDSC_mig' TumorArray{i}]).Units = 'cell/(milliliter*day)';
    params.(['k_MDSC_mig' TumorArray{i}]).Notes = 'estimated';
end
...
for i=2:3
    params.(['k_Mac_mig' TumorArray{i}]).Value = 3.5e5; %thein fitted 4.0e5 %1.7e5
    params.(['k_Mac_mig' TumorArray{i}]).Units = 'cell/(milliliter*day)';
    params.(['k_Mac_mig' TumorArray{i}]).Notes = 'estimated';
end

for i=4:4
    params.(['k_Mac_mig' TumorArray{i}]).Value = 3.0e5; %1.7e5
    params.(['k_Mac_mig' TumorArray{i}]).Units = 'cell/(milliliter*day)';
    params.(['k_Mac_mig' TumorArray{i}]).Notes = 'estimated';
end
% Death rate
...
for i=2:3
    params.(['k_M2_pol' TumorArray{i}]).Value = 1.1; % thein fitted 0.99 %0.25
    params.(['k_M2_pol' TumorArray{i}]).Units = '1/day';
    params.(['k_M2_pol' TumorArray{i}]).Notes = 'estimated';
end

for i=4:4
    params.(['k_M2_pol' TumorArray{i}]).Value = 0.4; % 0.25
    params.(['k_M2_pol' TumorArray{i}]).Units = '1/day';
    params.(['k_M2_pol' TumorArray{i}]).Notes = 'estimated';
end
