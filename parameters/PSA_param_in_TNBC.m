...
function params = PSA_param_in_TNBC(TumorArray,cancer_clones,n_T_specs)
...
params.initial_met_diameter.Median = log(1);
...
for nc=1:ncancer_clones
    ...
    params.(['k_' cancer_clones{nc} '_growth']).Median = log(0.0072); 
    params.(['k_' cancer_clones{nc} '_growth']).Sigma = 0.8; 
...
for i=2:ntumors
    for nc=1:ncancer_clones
        ...
        params.(['k_' cancer_clones{nc} '_growth' TumorArray{i}]).Median = log(0.0072); 
        params.(['k_' cancer_clones{nc} '_growth' TumorArray{i}]).Sigma = 0.8; 
...
for t=1:n_T_specs
    ...
    params.(['n_T' num2str(t) '_clones']).Median = log(63); 
...
for i=2:4
    ...
    params.(['k_Mac_mig' TumorArray{i}]).Median = log(1.7e5); 
