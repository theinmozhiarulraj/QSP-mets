...
function params = parameters_TNBC_mets(TumorArray,cancer_clones,n_T_specs)
...
for i=2:ntumors
    params.(['delay' TumorArray{i}]).Value = 100;
...
params.initial_met_diameter.Value = 0.2; % 0.2 cm =2mm
...
for nc=1:ncancer_clones
    params.(['k_' cancer_clones{nc} '_growth']).Value = 0.0072;
...
for i=2:ntumors
    for nc=1:ncancer_clones
        params.(['k_' cancer_clones{nc} '_growth' TumorArray{i}]).Value = 0.0072;
...
for i=2:4
    params.(['k_CD8_mig' TumorArray{i}]).Value = 5.8e-12; 
    params.(['k_CD8_mig' TumorArray{i}]).Units = '1/minute/cell';
    params.(['k_CD8_mig' TumorArray{i}]).Notes = 'estimated';
end

for i=5:5
    params.(['k_CD8_mig' TumorArray{i}]).Value = 5.8e-12; % 5.8e-12
    params.(['k_CD8_mig' TumorArray{i}]).Units = '1/minute/cell';
    params.(['k_CD8_mig' TumorArray{i}]).Notes = 'estimated';
end
...
for i=2:4
    params.(['k_Mac_mig' TumorArray{i}]).Value = 1.7e5; 
