...
function antigen = create_antigen(model,cancers,varargin)
...
for i = 1:length(cancers)
    ...
    %disp(sbioselect(model, 'Name', ['agconc_C' num2str(i) '_' num2str(ID)]).Value)
    antigen.concentration(i).Value = sbioselect(model, 'Name', ['agconc_C' num2str(i) '_' num2str(ID)]).Value;
    antigen.concentration(i).Units = sbioselect(model, 'Name', ['agconc_C' num2str(i) '_' num2str(ID)]).Units;
