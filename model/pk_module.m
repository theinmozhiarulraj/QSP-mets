...
function model = pk_module(model,species_name,TumorArray,LNArray,params,dose_type)

% Get Compartments
comp_C = sbioselect(model, 'Name', 'V_C'); 
comp_P = sbioselect(model, 'Name', 'V_P'); 
comp_T = sbioselect(model, 'Name', 'V_T'); %thein: not used anymore
comp_LN = sbioselect(model, 'Name', 'V_LN'); %thein: not used anymore
...
for array=1:ntumors
    % Tumour
    A_T = addspecies(sbioselect(model, 'Name', ['V_T' TumorArray{array}]),'A',0.0,'InitialAmountUnits','nanomolarity');
        set(A_T,'Notes',['Concentration of ' species_name ' in tumour compartment']);
end

for array=1:nLNs
    % Lymph Node
    A_LN = addspecies(sbioselect(model, 'Name', ['V_LN' uniqLNArray{array}]),'A',0.0,'InitialAmountUnits','nanomolarity');
...
for array=1:ntumors
    ...
    reaction = addreaction(model,['V_C.A <-> V_T' TumorArray{array} '.A']); % q_T = perm_CT_Ab*S_CT*Tum
        set(reaction,'ReactionRate',['q_T*(V_C.A/gamma_C - V_T' TumorArray{array} '.A/gamma_T)']);
        set(reaction,'Notes',[species_name ' diffusive transport to tumor compartment']);
    % Convective Transport: Tumour to Lymph Node
    reaction = addreaction(model,['V_T' TumorArray{array} '.A -> V_LN' LNArray{array} '.A']);
        set(reaction,'ReactionRate',['q_LD*V_T' TumorArray{array} '*V_T' TumorArray{array} '.A/gamma_T']);
        set(reaction,'Notes',[species_name ' convective transport from tumor to lymph node']);
end

for array=1:nLNs
    % Diffusive Transport: Central to Lymph Node
    reaction = addreaction(model,['V_C.A <-> V_LN' uniqLNArray{array} '.A']);
        set(reaction,'ReactionRate',['q_LN*(V_C.A/gamma_C - V_LN' uniqLNArray{array} '.A/gamma_LN)']);
        set(reaction,'Notes',[species_name ' diffusive transport to lymph node compartment']);
    % Convective Transport: Lymph Node to Central
    reaction = addreaction(model,['V_LN' uniqLNArray{array} '.A -> V_C.A']);
        set(reaction,'ReactionRate',['q_LD*V_T' TumorArray{array} '*V_LN' uniqLNArray{array} '.A/gamma_LN']);
...
for array=1:ntumors
    ...
    rename(sbioselect(comp_T,'Name','A'),species_name);
end

for array=1:nLNs
    comp_LN=sbioselect(model,'Name',['V_LN' uniqLNArray{array}]);
    rename(sbioselect(comp_LN,'Name','A'),species_name);
end
