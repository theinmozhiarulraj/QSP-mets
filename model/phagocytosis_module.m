...
function model = phagocytosis_module(model,params,TumorArray,LNArray,varargin)
...
for array=1:ntumors
    ...
    compDrug = sbioselect(model, 'Name', ['V_T' TumorArray{array}]); 
    gamma = 'gamma_T';
    
    % Add the synapse compartment
    comp = addcompartment(model,['syn_M_C' TumorArray{array}],params.A_syn.Value,'CapacityUnits',params.A_syn.Units);
...
if (antiCD47)
    ...
    for array=1:ntumors
        ...
        comp=sbioselect(model,'Name',['syn_M_C' TumorArray{array}]);    
        % Add Species
    ...
    model = pk_module(model,'aCD47',TumorArray,LNArray,params_aCD47);
...
for array=1:ntumors
    p = addparameter(model,['H_Mac_C' TumorArray{array}],0,'ValueUnits','dimensionless','ConstantValue',false);
        set(p,'Notes','Hill function for overall checkpoint inhibition on phagocytosis of cancer cell by macrophage ');
    p = addparameter(model,['H_PD1_M' TumorArray{array}],0,'ValueUnits','dimensionless','ConstantValue',false);
        set(p,'Notes','Hill function for PD1-mediated inhibition on phagocytosis of cancer cell by macrophage ');
    p = addparameter(model,['H_SIRPa' TumorArray{array}],0,'ValueUnits','dimensionless','ConstantValue',false);
...
for array=1:ntumors
   ...
   % Binding b/t CD47 and SIRPa in the synapse of cancell cell and macrophage in trans
    ...
    addrule(model,['H_SIRPa' TumorArray{array} ' = (' comp.Name '.CD47_SIRPa/SIRPa_50)^n_SIRPa/((' comp.Name '.CD47_SIRPa/SIRPa_50)^n_SIRPa + 1)'],'repeatedAssignment');
    addrule(model,['H_PD1_M' TumorArray{array} ' = ((' comp.Name '.PD1_PDL1+' comp.Name '.PD1_PDL2)/PD1_50)^n_PD1/(((' comp.Name '.PD1_PDL1+' comp.Name '.PD1_PDL2)/PD1_50)^n_PD1 + 1)'],'repeatedAssignment');
    addrule(model,['H_Mac_C' TumorArray{array} ' = 1 - (1-H_SIRPa' TumorArray{array} ')*(1-H_PD1_M' TumorArray{array} ')'],'repeatedAssignment');
