...
%         TumorName -- Suffix of tumor compartment name
% Output: rate    -- string containing rate of antigen release from dying cancer cells


function rate = antigen_rate(model,ID,cancers,P_C,nTcells,TumorName)
...
for i = 1:length(C)
    for k = 1:length(model.reaction)
        if strcmp(model.reaction(k).reaction, ['V_T' TumorName '.' C{i} ' -> V_T' TumorName '.C_x']) ...
    ...
    if strcmp(rate,'')
        ...
        rate = sprintf('%s_%s*(k_%s_death%s+k_%s_therapy%s)*V_T%s.%s+%s_%s*%s',...
            P,C{i},C{i},TumorName,C{i},TumorName,TumorName,C{i},P,C{i},R_T);
    else
        rate = sprintf('%s+%s_%s*(k_%s_death%s+k_%s_therapy%s)*V_T%s.%s+%s_%s*%s',...
            rate,P,C{i},C{i},TumorName,C{i},TumorName,TumorName,C{i},P,C{i},R_T);
    end
end
