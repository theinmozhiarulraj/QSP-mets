...
for i = 1:length(params_out.names)
    ...
    elseif strcmpi(params_out.(params_out.names{i}).Type , 'pre')
        for j = 1:n_PSA
            k = find(strcmp(simDataPSApost(index(j)).simData.DataNames, params_out.names{i} ));
            simDataPSAprep(index(j),i) = simDataPSApost(index(j)).simData.Data(1,k);
        end
    end
...
params_outPlaus = responseStatus_mets(simDataPSA,simDataPSApost,params_outPlaus); %thein
